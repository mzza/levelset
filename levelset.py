...
def PDE_update(X,LSF_init,iter,delta_t,mu,lambda_,epsilon,alpha):
	...
	while iter_time<=iter:
		if iter_time%50==1:
			seg_1D=(np.where(phi<=0,0,1)).astype('float32')
			seg=cv2.merge((seg_1D,seg_1D,seg_1D))
			plt.subplot(4,5,int(iter_time//50)+1)
			plt.imshow(seg)
...
class levelset(object):
	...
	def fit(self,X):
		...
		LSF_init[25:35,20:30]=-self.c
		LSF_init[25:35,40:50]=-self.c
